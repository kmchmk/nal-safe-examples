...
import { RemoveOwnerTxParams } from '@safe-global/protocol-kit'
...
async function createPropose(){
    ...
    const params: RemoveOwnerTxParams = {
        ownerAddress: OWNER_4_ADDRESS,
        threshold:2 // Optional. If `threshold` isn't provided the current threshold won't change.
      }
      const safeTransaction = await protocolKitOwner1.createRemoveOwnerTx(params)
